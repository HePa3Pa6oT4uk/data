...
def chunk_gen(a, b, c, d, side_x, side_y):
    ...
    return chunk
def generate_map(side, chunk_side,  max_height, num_points=100):
    ...
    map_1 = np.array([[0]*(side*chunk_side)]*(side*chunk_side))
    for x in range(side):
        for y in range(side):
            map_1[chunk_side*y:chunk_side*(y+1), chunk_side*x:chunk_side*(x+1)] = chunk_gen(heights_mid[x,y], heights_mid[x,y+1], heights_mid[x+1,y+1], heights_mid[x+1,y],chunk_side, chunk_side)
    return [map_1, x_new, y_new]
def generate_xor(a, b, c):
    map = generate_map(a, b, c)[0]
